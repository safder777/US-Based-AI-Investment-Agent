...
if submitted:
    with st.spinner("Generating your personalized investment plan..."):
        ...
        <h3>Investor Profile</h3><p>{result['profile']}</p>
        <h3>Research Plan</h3><p>{result['research_plan']}</p>
        {f'<h3>Market Data</h3><p>{result["market_data"]}</p>' if scope == 'comprehensive' else ''}
        {f'<h3>Macro Analysis</h3><p>{result["macro_analysis"]}</p>' if scope == 'comprehensive' else ''}
        <h3>Portfolio</h3><p>{result['portfolio']}</p>
        <h3>Final Proposal</h3><p>{result['proposal']}</p>
        """
        with open("proposal.html", "w", encoding="utf-8") as f:
            f.write(pdf_html)
        pdfkit.from_file("proposal.html", "proposal.pdf")
        with open("proposal.pdf", "rb") as f:
            st.download_button("📥 Download Proposal as PDF", data=f, file_name="investment_proposal.pdf")
